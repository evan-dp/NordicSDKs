...
static void thread_stack_init(void)
{
    ...
    {
        error = otLinkSetChannel(p_instance, THREAD_CHANNEL);
        ASSERT(error == OT_ERROR_NONE);

        error = otLinkSetPanId(p_instance, THREAD_PANID);
        ASSERT(error == OT_ERROR_NONE);
    }
    ...
    uint32_t err_code = bsp_indication_set(BSP_THREAD_ENABLE);
    APP_ERROR_CHECK(err_code);
...
static void thread_stack_deinit(void)
{
    ...
    otError error = otPlatUartDisable();
    ASSERT(error == OT_ERROR_NONE);
